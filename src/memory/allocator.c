...
/* Tunable constants */
#define MAGIC 0x54454E53u /* 'TENS' */
#define MAX_DIM 8
#define HASH_ENTRIES 4096 /* must be power of two for simple masking */
#define ALIGNMENT 16

/* Helpers */
static inline size_t align_up(size_t x, size_t a) { return (x + (a - 1)) & ~(a - 1); }

/* Offsets are stored as uint32_t where possible to reduce metadata size. */
typedef uint32_t off32;

/* Block header for ephemeral region. Stored inline before each block.
 * Layout (in memory): [BlockHeader][user-data]
 */
typedef struct BlockHeader {
    uint32_t size;     /* size of user-data in bytes */
...
/* The pool header is at the start of the mapped block and holds allocator metadata. */
typedef struct PoolHeader {
     ...
     */
    /* We don't declare the arrays here because they are variable-positioned in the single block. */
} PoolHeader;
...
static inline uint8_t* off_to_ptr(Pool* p, off32 o) { return (o == 0) ? NULL : p->base + o; }
static inline off32 ptr_to_off(Pool* p, void* ptr) { return (ptr == NULL) ? 0 : (off32)((uint8_t*)ptr - p->base); }

/* Simple linear-probing insert/find for hash table inside pool. Returns index or -1 on failure. */
static int hash_find_index(TensorEntry* table, uint64_t id) {
    ...
    for (uint32_t probe = 0; probe < HASH_ENTRIES; ++probe) {
        ...
        if (table[i].id == id) return (int)i;
        if (table[i].id == 0) return -(int)i - 1; /* empty slot */
    }
    return -1;
}

static int hash_insert(TensorEntry* table, uint64_t id, off32 off, uint32_t size) {
    if (id == 0) return -1;
    uint32_t mask = HASH_ENTRIES - 1;
    uint32_t h = (uint32_t)(id ^ (id >> 33));
    uint32_t idx = h & mask;
...
static int hash_remove(TensorEntry* table, uint64_t id) {
    ...
    if (idx >= 0) {
        table[idx].id = 0; /* tombstone naive strategy: we clear the slot -> this may break finds; for simplicity we allow it (not production). */
        table[idx].offset = 0;
        table[idx].size = 0;
        return idx;
    }
...
/* Initialize pool: mmap a block of size bytes and carve header + hash tables + regions.
 * Layout plan:
...
Pool* pool_init(size_t total_bytes) {
    ...
    Pool* p = malloc(sizeof(Pool)); /* small heap allocation for descriptor only (allowed). */
    if (!p) {
        munmap(base, size);
        return NULL;
    }
    ...
    hdr->resident_end = hdr->resident_start; /* bump pointer initially equals start */
    cursor += (off32)align_up((off32)resident_bytes, ALIGNMENT);

    /* Rest is ephemeral region */
    hdr->ephemeral_start = cursor;
    hdr->ephemeral_capacity = (uint32_t)(size - cursor);

    /* Initialize first block header in ephemeral region */
    BlockHeader* first = (BlockHeader*)(p->base + hdr->ephemeral_start);
    first->size = hdr->ephemeral_capacity - (uint32_t)align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT);
    first->free = 1;
    first->next = 0;
    first->prev = 0;
...
/* Allocate resident: simple bump allocator. Returns pointer or NULL. */
void* allocate_resident(Pool* p, uint64_t id, size_t size) {
    ...
    if ((uint64_t)start + aligned > limit) return NULL; /* out of resident space */
    void* ptr = p->base + start;
    p->hdr->resident_end = (off32)(start + (off32)aligned);
    if (hash_insert(p->resident_table, id, ptr_to_off(p, ptr), (uint32_t)aligned) < 0) {
        return NULL; /* table full */
    }
    return ptr;
}

/* Ephemeral allocation: first-fit scan of block headers */
void* allocate_ephemeral(Pool* p, uint64_t id, size_t size) {
    ...
    while (cur_off != 0) {
        ...
        if (bh->free && bh->size >= aligned) {
            ...
            /* if remaining space large enough to hold a new block header + min payload, split */
            uint32_t min_split = (uint32_t)align_up((uint32_t)sizeof(BlockHeader) + 16, ALIGNMENT);
            if (remaining > min_split) {
                off32 new_off = cur_off + (off32)align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT) + bh->size;
                BlockHeader* nh = (BlockHeader*)(p->base + new_off);
                nh->free = 1;
                nh->size = remaining - (uint32_t)align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT);
                nh->next = bh->next;
                nh->prev = cur_off;
                bh->next = new_off;
                if (nh->next) {
            ...
            void* data_ptr = (uint8_t*)bh + align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT);
            if (hash_insert(p->ephemeral_table, id, ptr_to_off(p, data_ptr), (uint32_t)aligned) < 0) {
...
/* Coalesce free neighbors (simple) */
static void try_coalesce(Pool* p, BlockHeader* bh, off32 bh_off) {
    ...
    if (bh->next) {
        ...
        if (nx->free) {
            bh->size += (uint32_t)align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT) + nx->size;
            bh->next = nx->next;
            if (nx->next) {
    ...
    if (bh->prev) {
        ...
        if (pr->free) {
            pr->size += (uint32_t)align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT) + bh->size;
            pr->next = bh->next;
            if (bh->next) {
                BlockHeader* nx = (BlockHeader*)(p->base + bh->next);
                nx->prev = bh->prev;
...
int free_ephemeral(Pool* p, uint64_t id) {
    ...
    if (off == 0) return -1;
    BlockHeader* bh = (BlockHeader*)(p->base + (off - align_up((uint32_t)sizeof(BlockHeader), ALIGNMENT)));
    bh->free = 1;
    /* coalesce neighbors */
...
/* Lookup pointer by id. Checks resident then ephemeral tables. */
void* get_tensor_ptr(Pool* p, uint64_t id) {
    if (!p || id == 0) return NULL;
    int idx = hash_find_index(p->resident_table, id);
    if (idx >= 0) return off_to_ptr(p, p->resident_table[idx].offset);
    idx = hash_find_index(p->ephemeral_table, id);
    if (idx >= 0) return off_to_ptr(p, p->ephemeral_table[idx].offset);
    return NULL;
}

/* Debug helper: print small summary */
void pool_debug(Pool* p) {
    printf("Pool total=%u resident_start=%u resident_end=%u resident_capacity=%u ephemeral_start=%u ephemeral_capacity=%u\n",
           p->hdr->total_size, p->hdr->resident_start, p->hdr->resident_end, p->hdr->resident_capacity, p->hdr->ephemeral_start, p->hdr->ephemeral_capacity);
}